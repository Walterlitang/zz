        ...
        var url = decodeURIComponent(strPath);
        var start = url.lastIndexOf("/");
        var end = url.lastIndexOf(".");
        var file = url.slice(start + 1, end);
...
------------`;
        ...
        arr.forEach((item, index) => {
            if (item.indexOf(file) == -1) {
                newStr += `<p> ${item.trim()}</p>`;
            } else {
                newStr += `<div class= 'text-head' > <span class='title'>${item.trim()}</span></div>`;
            }
        })
        newStr += "</div>";
        var wrap = document.querySelector(".wrap");
        wrap.innerHTML = newStr;
